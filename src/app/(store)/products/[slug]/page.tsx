...
export default async function ProductPage({ params }: { params: { slug: string } }) {
  ...
  // A product can have multiple categories — use the first as the "primary"
  // one for the breadcrumb and related-products lookup
  const primaryCategory = product.categories[0]?.category ?? null;
  const allCategoryIds = product.categories.map((pc) => pc.categoryId);

  // Related products — share any of the same categories, up to 4
  const related = allCategoryIds.length > 0
    ...
    <div className="max-w-8xl mx-auto px-4 sm:px-6 py-10 page-enter">
      ...
      <nav className="text-xs text-stone-400 font-sans mb-6 flex items-center gap-1.5">
        ...
        {primaryCategory && (
          <>
            <span>/</span>
            <a href={`/products?category=${primaryCategory.slug}`} className="hover:text-souk-700 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
        <ProductDetailClient
          ...
          waInquiry={waInquiry}
          categoryName={primaryCategory?.name ?? "Souk Fashion House"}
