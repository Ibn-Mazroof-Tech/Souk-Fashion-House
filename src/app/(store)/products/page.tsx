...
type SearchParams = {
  ...
  page?: string;
  featured?: string;
  color?: string; // comma-separated variant colors
};
...
async function getProducts(params: SearchParams) {
  ...
  // Category filter — if it's a parent (top-level nav item), include its
  // sub-categories' products too; if it's a sub-category itself, match only that.
  let matchedCategory: { id: string; name: string; children: { id: string }[] } | null = null;
  if (categorySlug !== "all") {
    ...
    if (matchedCategory) {
      const categoryIds =
        matchedCategory.children.length > 0
          ? [matchedCategory.id, ...matchedCategory.children.map((c) => c.id)]
          : [matchedCategory.id];
  ...
  const [products, total, colorRows] = await Promise.all([
    prisma.product.findMany({
      where, orderBy,
      skip: (page - 1) * LIMIT,
      take: LIMIT,
      include: { categories: { include: { category: true } }, _count: { select: { reviews: true, variants: true } } },
    }),
  ...
  return { products, total, colors: colorRows, page, pages: Math.ceil(total / LIMIT), matchedCategoryName: matchedCategory?.name };
}

export default async function ProductsPage({ searchParams }: { searchParams: SearchParams }) {
  const [categories, { products, total, colors, page, pages, matchedCategoryName }] = await Promise.all([
    // Parent categories only, each with their children — for the filter tabs
    ...
    <div className="max-w-8xl mx-auto px-4 sm:px-6 py-10 page-enter">
      ...
      <div className="mb-8">
        <h1 className="font-display text-4xl md:text-5xl font-medium text-stone-900 mb-2">
          {category === "all" ? "All Collections" : matchedCategoryName ?? category}
        </h1>
