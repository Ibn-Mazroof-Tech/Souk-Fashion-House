...
async function getHomeData() {
  const [featuredProducts, categories, heroSlides, testimonials] = await Promise.all([
    ...
    // Top-level categories only — each with its children, so we can count
    // products across the whole branch (a parent may have zero direct
    // products if everything lives under its sub-categories)
    prisma.category.findMany({
      where: { parentId: null },
      orderBy: { sortOrder: "asc" },
      include: {
        children: { include: { _count: { select: { products: { where: { product: { active: true } } } } } } },
  ...
  return { featuredProducts, categories, heroSlides, testimonials };
}

export default async function HomePage() {
  const { featuredProducts, categories, heroSlides, testimonials } = await getHomeData();
    ...
    <div className="page-enter">
      ...
      <section className="relative bg-gradient-to-br from-cream-100 via-cream-50 to-white overflow-hidden">
        ...
        <div className="max-w-8xl mx-auto px-4 sm:px-6 py-16 md:py-24">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            {/* Hero carousel — admin-managed via Admin → Homepage */}
            {heroSlides.length > 0 ? (
              <HeroCarousel slides={heroSlides} />
            ) : (
              <div className="relative rounded-2xl overflow-hidden aspect-[4/5] md:aspect-[3/4] shadow-souk-md bg-gradient-to-br from-souk-100 to-cream-100 flex items-center justify-center">
                <p className="text-souk-700/50 text-sm font-sans">Add hero images in Admin → Homepage</p>
              </div>
            )}
          </div>
        </div>
      ...
      <section className="max-w-8xl mx-auto px-4 sm:px-6 py-14">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          >
            ...
            <div className="relative">
              ...
              <p className="text-souk-200 text-sm font-sans">
                {categories.reduce(
                  (sum, c) => sum + c._count.products + c.children.reduce((s, ch) => s + ch._count.products, 0),
                  0
                )} products
              </p>
            </div>
          ...
          {/* Dynamic top-level categories — counts include their sub-categories */}
          {categories.map((cat) => {
            const totalProducts = cat._count.products + cat.children.reduce((s, ch) => s + ch._count.products, 0);
            return (
              <Link
                key={cat.id}
                href={`/products?category=${cat.slug}`}
                className="group relative rounded-2xl overflow-hidden aspect-square shadow-souk-sm hover:shadow-souk-md transition-shadow bg-cream-100"
              >
                <img
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  ...
                  <p className="text-stone-200 text-xs font-sans">{totalProducts} products</p>
                </div>
              </Link>
            );
          })}
      ...
      {/* ── Testimonials — admin-managed via Admin → Homepage ── */}
      {testimonials.length > 0 && (
        <section className="max-w-8xl mx-auto px-4 sm:px-6 py-14">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.map(({ id, name, location, rating, text }) => (
              <div key={id} className="bg-white rounded-2xl p-6 border border-stone-100 shadow-souk-sm">
