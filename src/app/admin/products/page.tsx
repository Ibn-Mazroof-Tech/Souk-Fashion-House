...
type Category = { id: string; name: string; slug: string; parentId: string | null; children?: Category[]; _count?: { products: number } };

const SIZE_OPTIONS = ["XS", "S", "M 38", "L 40", "XL 42", "XXL 44", "XXXL 46", "Free"];
const EMPTY_FORM = {
  name: "", description: "", price: "", mrp: "", stock: "0",
  featured: false, active: true, images: [] as string[],
  sizes: [] as string[], categoryIds: [] as string[],
...
export default function AdminProductsPage() {
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setCategories((cats) => [...cats, { ...data.data, children: [], _count: { products: 0 } }].sort((a, b) => a.name.localeCompare(b.name)));
        setForm((f) => ({ ...f, categoryIds: [...f.categoryIds, data.data.id] }));
  ...
  const handleEdit = (p: Product) => {
    ...
    setForm({
      ...
      images: p.images, sizes: p.sizes, categoryIds: p.categories.map((pc) => pc.category.id),
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-souk-100 shadow-souk-md p-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            {/* Categories — multi-select checkbox tree. A product can be
                tagged under several categories at once (e.g. a Kashmiri kurti
                could sit under both "Kurtis > Kashmiri" and "Collections"). */}
            <div>
              <label className="label-xs">Categories * <span className="text-stone-400 font-normal normal-case">(pick one or more)</span></label>
              <div className="border border-stone-200 rounded-xl p-3 max-h-56 overflow-y-auto space-y-2.5">
                {categories.map((c) => (
                  <div key={c.id}>
                    <label className="flex items-center gap-2 text-sm font-medium text-stone-700 font-sans cursor-pointer">
                      <input
                        type="checkbox"
                        checked={form.categoryIds.includes(c.id)}
                        onChange={() => toggleCategory(c.id)}
                        className="w-4 h-4 rounded border-stone-300 text-souk-700 focus:ring-souk-700"
                      />
                      {c.name}
                    </label>
                    {c.children && c.children.length > 0 && (
                      <div className="ml-6 mt-1.5 space-y-1.5">
                        {c.children.map((sub) => (
                          <label key={sub.id} className="flex items-center gap-2 text-xs text-stone-600 font-sans cursor-pointer">
                            <input
                              type="checkbox"
                              checked={form.categoryIds.includes(sub.id)}
                              onChange={() => toggleCategory(sub.id)}
                              className="w-3.5 h-3.5 rounded border-stone-300 text-souk-700 focus:ring-souk-700"
                            />
                            {sub.name}
                          </label>
                        ))}
              ...
              {!showNewCategory ? (
                <button
                  ...
                  className="flex items-center gap-1.5 mt-2 text-xs font-medium text-souk-700 hover:bg-souk-50 px-2 py-1.5 rounded-lg transition-colors"
                >
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex flex-wrap gap-1">
                        {p.categories.slice(0, 2).map(({ category }) => (
                          <span key={category.id} className="text-xs bg-souk-50 text-souk-700 px-2 py-1 rounded-full font-medium font-sans">
                            {category.name}
                          </span>
                        ))}
                        {p.categories.length > 2 && (
                          <span className="text-xs bg-stone-100 text-stone-500 px-2 py-1 rounded-full font-medium font-sans">
                            +{p.categories.length - 2}
                          </span>
                        )}
                        {p.categories.length === 0 && (
                          <span className="text-xs text-stone-400 font-sans italic">Uncategorized</span>
                        )}
                      </div>
                    </td>
                    <td className="px-5 py-3.5">
