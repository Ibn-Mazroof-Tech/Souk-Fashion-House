...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { price, mrp, variants, colorImages, categoryIds, ...rest } = result.data;
    ...
    // approach for colorImages and categoryIds.
    const product = await prisma.$transaction(async (tx) => {
      ...
      return tx.product.update({
        ...
        include: {
          categories: { include: { category: true } }, variants: true, colorImages: true,
        },
      });
    });
