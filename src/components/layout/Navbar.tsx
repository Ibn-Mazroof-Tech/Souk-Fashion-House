...
import { ShoppingBag, Menu, X, User, LogOut, Package, Heart, Settings, ChevronDown } from "lucide-react";
...
export function Navbar({ categories = [] }: { categories?: NavCategory[] }) {
  // Each top-level category (Collections, Kurtis, Shawls, Suits, ...) is a
  // real category the admin creates — no hardcoded "browse everything" link
    ...
    >
      <div className="max-w-8xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          {/* ── Desktop nav links — hover mega-menu for items with sub-categories ── */}
            ...
            {navLinks.map((link) => (
              <div key={link.href} className="relative group">
                <Link
                  href={link.href}
                  className={cn(
                    "px-3.5 py-2 rounded-lg text-sm font-medium transition-colors font-sans inline-block",
                    pathname === "/products" &&
                      (currentCategorySlug === link.slug || link.children.some((c) => c.slug === currentCategorySlug))
                ...
                {link.children.length > 0 && (
                  <div className="absolute left-0 top-full pt-1 hidden group-hover:block z-40">
                    <div className="bg-white border border-stone-100 rounded-xl shadow-souk-md py-1.5 min-w-[160px] animate-slide-up">
                      {link.children.map((sub) => (
                        <Link
                          key={sub.slug}
                          href={`/products?category=${sub.slug}`}
                          className="block px-4 py-2 text-sm text-stone-600 hover:text-souk-700 hover:bg-souk-50 transition-colors font-sans"
                        >
                          {sub.name}
                        </Link>
                      ))}
            ...
            {navLinks.map((link) =>
              link.children.length > 0 ? (
                <details key={link.href} className="group">
                  <summary className="flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-medium text-stone-700 hover:bg-stone-50 cursor-pointer list-none">
                  ...
                  <div className="pl-4 space-y-0.5 py-1">
                    {link.children.map((sub) => (
                      <Link
                        ...
                        className="flex items-center px-3 py-2 rounded-lg text-sm text-stone-500 hover:bg-stone-50 hover:text-souk-700"
                      >
                        {sub.name}
                      </Link>
                    ))}
